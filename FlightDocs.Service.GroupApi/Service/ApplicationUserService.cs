...
namespace FlightDocs.Service.GroupApi.Service
{
    public class ApplicationUserService : IApplicationUserService
    {
        ...
        {
            var client = _httpClientFactory.CreateClient("ApplicationUser");
            var response = await client.GetAsync($"/api/ApplicationUserApi").ConfigureAwait(false);
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                try
                {
                    var resp = JsonConvert.DeserializeObject<IEnumerable<ApplicationUserDto>>(apiContent);
                    return resp;
                }
                catch (Exception ex)
                {
